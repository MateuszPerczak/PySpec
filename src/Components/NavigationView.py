...
class NavigationView(ttk.Frame):
    def __init__(self: object, parent: object, theme: object) -> ttk.Frame:
        ...
        ttk.Frame(self.navbar, style='dark.TFrame').pack(
        ...
        self.header_label: ttk.Label = ttk.Label(
            self.header, text='', style='big.TLabel', compound='left')
        self.header_label.pack(side='left', anchor='center', padx=(10, 0))
        self.header.pack(side='top', fill='x', ipady=10)
        # scrollable frame
        self.canvas: Canvas = Canvas(
            self, bg=self.master['background'], bd=0, highlightthickness=0, yscrollcommand=scrollbar.set, takefocus=False)
    ...
    def add_item(self: object, item: object, props: dict = {}) -> None:
        ...
        # create radiobutton
        self.radiobuttons[item.name] = ttk.Radiobutton(self.navbar, image=self.icon_cache[item.name][self.theme.get_theme(
        )], text=item.name, compound='left', value=item.name, variable=self.nav_selection, command=self.__show_panel)
        self.radiobuttons[item.name].pack(
            side=item.side, fill='x', padx=10, pady=(10, 0))
        # store page in dict
    ...
    def __show_panel(self: object) -> None:
        ...
        self.header_label.configure(text=selected_panel.capitalize(
        ), image=self.icon_cache[selected_panel][self.theme.get_theme()])
        # move content to top
        self.canvas.yview_moveto(0)

    def __on_wheel(self: object, event: Event) -> None:
        self.canvas.yview_scroll(
            int(-self.acceleration*(event.delta/120)), 'units')
    ...
    def __on_theme_changed(self: object, theme: str) -> None:
        ...
        for button in self.radiobuttons:
            self.radiobuttons[button].configure(
                image=self.icon_cache[button][theme])
        # update header
        selected_panel: str = self.nav_selection.get()
        self.header_label.configure(text=selected_panel.capitalize(
        ), image=self.icon_cache[selected_panel][self.theme.get_theme()])
